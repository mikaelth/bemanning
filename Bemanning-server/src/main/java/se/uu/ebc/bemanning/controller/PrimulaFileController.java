...
@Slf4j
@Controller
@RequestMapping("/files/primula")
public class PrimulaFileController {
    ...
    @Autowired
    private PrimulaExcelService primulaExcelService;
/* 
...
//			model.addAttribute("years",edRepo.getYears());
    ...
    public String requestUpdateRegsFromCSV(Model model, HttpServletRequest request, HttpServletResponse response, final FormFileDataVO formValues) throws IOException{
        ...
        Files.write(path, formValues.getExcelFile().getBytes());
 		...
 		model.addAttribute( "primulaEntries",primulaExcelService.getExcelDataAsList(formValues.isIgnoreExistingValues()) );
		log.debug("Model: {}",model);

        return "PrimulaBulkUpload";
    }
