...
public interface CourseInstanceRepo extends JpaRepository<CourseInstance, Long>, JpaSpecificationExecutor<CourseInstance>{
    ...
    // findById(Long id) is inherited from JpaRepository and returns Optional<CourseInstance>

	@Query("SELECT ci FROM CourseInstance AS ci WHERE ci.year = ?1 AND ci.course.code= ?2 order by ci.course.courseGroup, ci.course.code")
	public List<CourseInstance> findByYearAndCourseCode(String year, String courseCode);
