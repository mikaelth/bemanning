...
//			Set<Staff> staff = staffRepo.findByYear(thisYear);
    ...
    public String viewByCourse(@RequestParam(value = "year", required = false) String year, Model model, Principal principal, HttpServletRequest request) {
		try {
			log.debug("viewByCourse, year "+ year);
			log.debug("viewByCourse, model "+ReflectionToStringBuilder.toString(model, ToStringStyle.MULTI_LINE_STYLE));
			log.debug("viewByCourse, principal "+ReflectionToStringBuilder.toString(principal, ToStringStyle.MULTI_LINE_STYLE));
			...
			log.debug("viewByCourse, thisYear "+ thisYear);
			...
			for (CourseInstance ci : cis) {
				log.debug("course "+ci.getCourse().getCode() + " " + ci.getCourse().getSeName());
//				log.debug("viewByCourse, ci "+ReflectionToStringBuilder.toString(ci, ToStringStyle.MULTI_LINE_STYLE));
			}
			log.debug("viewByCourse, ous "+ReflectionToStringBuilder.toString(staff.getOrganisationUnit().getEconomyHolder(thisYear).getExpandedOuAbbreviations(thisYear), ToStringStyle.MULTI_LINE_STYLE));
			log.debug("viewByCourse, staff "+ReflectionToStringBuilder.toString(staff, ToStringStyle.MULTI_LINE_STYLE));
			log.debug("viewByCourse, budgetYear "+ReflectionToStringBuilder.toString(new BudgetYear(thisYear, staffRepo.getStaffedYears()), ToStringStyle.MULTI_LINE_STYLE));
			log.debug("viewByCourse, budgetDept "+ReflectionToStringBuilder.toString(budgetDept, ToStringStyle.MULTI_LINE_STYLE));
			...
			if (log.isErrorEnabled()) {
				log.error("viewByCourse, pesky exception "+e);
			}
           return "{\"ERROR\":"+e.getMessage()+"\"}";
        }
	}
    ...
    public String viewByCoursePost(@ModelAttribute("year") String year, Model model, Principal principal, HttpServletRequest request) {
			if (log.isDebugEnabled()) {
				log.debug("viewByCourse, POST year "+ year);
				log.debug("viewByCourse, POST model "+ReflectionToStringBuilder.toString(model, ToStringStyle.MULTI_LINE_STYLE));
			}
...
//			model.addAttribute("akka", akkaService);
    ...
    public String viewStaffSummaryPost(@ModelAttribute("year") String year, @ModelAttribute("ou") Long ouid, Model model, Principal principal, HttpServletRequest request) {
				log.debug("viewStaffSummary, POST year "+ year);
				log.debug("viewStaffSummary, POST ou "+ ouid);
...
/*
    ...
    public String viewProgressByPhD(@RequestParam(value = "includedstudents", required = false, defaultValue = "active") String included, Model model, Principal principal, HttpServletRequest request) {
		try {
			log.debug("viewProgressByPhD, model "+ReflectionToStringBuilder.toString(model, ToStringStyle.MULTI_LINE_STYLE));
			log.debug("viewProgressByPhD, principal "+ReflectionToStringBuilder.toString(principal, ToStringStyle.MULTI_LINE_STYLE));
			log.debug("viewProgressByPhD, october "+ Calendar.OCTOBER);
...
//			model.addAttribute("phdStudents", phdRepo.findAll());
			...
			if (log.isErrorEnabled()) {
				log.error("viewProgressByPhD, pesky exception "+e);
			}
           return "{\"ERROR\":"+e.getMessage()+"\"}";
        }
	}
    ...
    public String viewCourseEconOverview(@RequestParam(value = "year", required = false) String year, Model model, Principal principal, HttpServletRequest request) {
		try {
			log.debug("viewCourseEconOverview, year "+ year);
			log.debug("viewCourseEconOverview, model "+ReflectionToStringBuilder.toString(model, ToStringStyle.MULTI_LINE_STYLE));
			log.debug("viewCourseEconOverview, principal "+ReflectionToStringBuilder.toString(principal, ToStringStyle.MULTI_LINE_STYLE));
			...
			log.debug("viewCourseEconOverview, courses "+ReflectionToStringBuilder.toString(cis, ToStringStyle.MULTI_LINE_STYLE));
			...
			if (log.isErrorEnabled()) {
				log.error("viewCourseEconOverview, pesky exception "+e);
			}
           return "{\"ERROR\":"+e.getMessage()+"\"}";
        }
	}
    ...
    public String viewCourseOverview(@RequestParam(value = "year", required = false) String year, Model model, Principal principal, HttpServletRequest request) {
		try {
			log.debug("viewCourseOverview, year "+ year);
			log.debug("viewCourseOverview, model "+ReflectionToStringBuilder.toString(model, ToStringStyle.MULTI_LINE_STYLE));
			log.debug("viewCourseOverview, principal "+ReflectionToStringBuilder.toString(principal, ToStringStyle.MULTI_LINE_STYLE));

			String thisYear = year==null ? thisYear() : year;

			List<CourseInstance> cis= ciRepo.findByYear(thisYear);

			log.debug("viewCourseOverview, courses "+ReflectionToStringBuilder.toString(cis, ToStringStyle.MULTI_LINE_STYLE));
			...
			if (log.isErrorEnabled()) {
				log.error("viewCourseOverview, pesky exception "+e);
			}
           return "{\"ERROR\":"+e.getMessage()+"\"}";
        }
	}
